...
/* returns FOUND if an item is an element of the priority queue */
...
/* searches for an items priority and return accordingly */
int findPriority(struct pq *pq, void *item) {
  int i;

  for (i=0; i < pq->count; i++) {
    if ((pq->queue)[i] == item) {
      return pq->priorities[i];
    }
  }

  /* returns NOTFOUND if, item is not in priority queue */
  return NOTFOUND;

}

/* Searches for an item and updates its priority accordingly */
